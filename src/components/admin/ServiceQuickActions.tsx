...
import { setServicePublished, setServiceSortOrder } from "@/actions/admin/services";
import { useAdminToast } from "@/components/admin/ui/toast";

export function ServicePublishedToggle({ id, published }: { id: string; published: boolean }) {
    ...
    <button
      ...
      onClick={() => {
        startTransition(async () => {
          const r = await setServicePublished(id, !published);
          if (!r.ok) {
...
export function ServiceSortOrderInput({ id, value }: { id: string; value: number }) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
    ...
    <div className="flex items-center justify-end gap-2">
      ...
      <button
        ...
        onClick={() => {
          ...
          startTransition(async () => {
            const r = await setServiceSortOrder(id, n);
